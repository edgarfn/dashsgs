  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-8 px-6 py-12">
      ...
      >
        ...
        <div className="space-y-1.5">
          <label htmlFor="filtro-categoria" className="block text-xs text-slate-400">
            Categoria
          </label>
          <select
            id="filtro-categoria"
          ...
          >
            ...
            {AUDIT_CATEGORIAS.map((categoria) => (
              <option key={categoria} value={categoria} className="bg-slate-900">
        ...
        <div className="space-y-1.5">
          <label htmlFor="filtro-acao" className="block text-xs text-slate-400">
            Evento
          </label>
          <select
            id="filtro-acao"
            ...
            className="max-w-[16rem] rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-100 outline-none focus:border-sky-400/60"
          >
            <option value="" className="bg-slate-900">
              Todos
            </option>
            ...
            {(acoes.data ?? []).map((acao) => (
              <option key={acao} value={acao} className="bg-slate-900">
                {rotuloDaAcao(acao)}
              </option>
            ))}
          </select>
        ...
        <div className="space-y-1.5">
          <label htmlFor="filtro-resultado" className="block text-xs text-slate-400">
            Resultado
          </label>
          <select
            id="filtro-resultado"
          ...
          >
            ...
            {AUDIT_RESULTADOS.map((resultado) => (
              <option key={resultado} value={resultado} className="bg-slate-900">
        ...
        {membros.data ? (
          <div className="space-y-1.5">
            <label htmlFor="filtro-ator" className="block text-xs text-slate-400">
              Pessoa
            </label>
            <select
              id="filtro-ator"
              ...
              className="max-w-[14rem] rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-100 outline-none focus:border-sky-400/60"
            >
              <option value="" className="bg-slate-900">
                Todas
              </option>
              {membros.data.map((membro) => (
                <option key={membro.user.id} value={membro.user.id} className="bg-slate-900">
                  {membro.user.name}
                </option>
              ))}
            </select>
        ...
        <button
          type="submit"
          className="rounded-lg bg-sky-500/90 px-4 py-2 text-sm font-medium text-slate-950 hover:bg-sky-400"
        >
          Aplicar
        </button>
        ...
        <Link
          ...
          className="px-2 py-2 text-sm text-slate-400 underline-offset-4 hover:text-slate-200 hover:underline"
        >
          Limpar
        </Link>
      </form>
                ...
                {dados.data.map((evento) => (
                  <tr key={evento.id} className="align-top">
                    <td className="px-4 py-3">
                      {evento.ator ? (
                        <>
                          <span className="block text-slate-200">{evento.ator.nome}</span>
                          <span className="block text-xs text-slate-500">{evento.ator.email}</span>
                        </>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      ...
                      <span className="block font-mono text-xs text-slate-500">{evento.acao}</span>
                      {detalhes(evento.changes).length > 0 ? (
                        <details className="mt-1 text-xs text-slate-500">
                          <summary className="cursor-pointer text-slate-400">Detalhes</summary>
                          <dl className="mt-1 space-y-0.5">
                            {detalhes(evento.changes).map(([chave, valor]) => (
                              <div key={chave} className="flex gap-2">
                                <dt className="text-slate-500">{chave}:</dt>
                                <dd className="break-all text-slate-400">{valor}</dd>
                              </div>
                            ))}
                          </dl>
                        </details>
                    ...
                    <td
                      className={`px-4 py-3 ${CORES_DO_RESULTADO[evento.resultado] ?? 'text-slate-300'}`}
                    >
      ...
      <p className="text-xs text-slate-500">
        A trilha é <strong className="font-medium text-slate-400">append-only</strong>: a aplicação
